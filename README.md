...
The `--install-root` flag is optional for all three package build scripts. When omitted, the script builds and stages the project automatically, equivalent to running `assemble-install-root.sh` first. Passing a pre-built root is useful in CI where the install root is shared across multiple package formats.

Quick one-step local builds:
```bash
packaging/release/build-appimage.sh --output ~/Downloads/
packaging/release/build-deb.sh --output ~/Downloads/
packaging/release/build-rpm.sh --output ~/Downloads/
```
